...
class ExpandingBottomSheet extends StatefulWidget {
  const ExpandingBottomSheet({
    ...
    this.builderThumnail,
    this.widthForCartIcon,
    this.cartHeight,
  });

  final double? widthForCartIcon;
...
class ExpandingBottomSheetState extends State<ExpandingBottomSheet>
  ...
  double get widthForCartIcon => widget.widthForCartIcon ?? _kWidthForCartIcon;
  double get cartHeight => widget.cartHeight ?? _kCartHeight;
  ...
  late double _width = widthForCartIcon;
...
//  Animation<double> _cartOpacityAnimation;
  ...
  bool get isCustomCart => widget.builderThumnail != null;
  ...
  Animation<double> _getHeightAnimation(double screenHeight) {
    if (_controller.status == AnimationStatus.forward) {
      ...
      return _getEmphasizedEasingAnimation(
        begin: cartHeight,
        peak: cartHeight + (screenHeight - cartHeight) * _kPeakVelocityProgress,
        end: screenHeight,
        isForward: true,
        parent: _controller.view,
      );
...
//  }
  ...
  Widget _buildThumbnails(int numProducts) {
    ...
    if (widget.builderThumnail != null) {
      return widget.builderThumnail!(totalCart);
    }
...
class ExtraProductsNumber extends StatelessWidget {
  ...
  // thumbnails).
  int _calculateOverflow(Map<String, int?> productMap) {
  ...
  Widget _buildOverflow(productsInCart, BuildContext context) {
    if (productsInCart.length > 3) {
      var numOverflowProducts = _calculateOverflow(productsInCart);
      ...
      return const SizedBox(); // build() can never return null.
    }
  }

  @override
  Widget build(BuildContext context) {
    return Selector<CartModel, Map<String, int?>>(
      selector: (_, cartModel) => cartModel.productsInCart,
      builder: (builder, productsInCart, child) =>
          _buildOverflow(productsInCart, context),
    );
  }
}
